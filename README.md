...
<a href="http://ec2-3-110-185-149.ap-south-1.compute.amazonaws.com" target="_blank">
  <img src="https://img.shields.io/badge/🚀_CLICK_HERE_FOR_LIVE_DEPLOYMENT-Success-48BB78?style=for-the-badge&labelColor=2F855A&color=48BB78" alt="Live Deployment" height="50"/>
</a>

<br><br>

[Features](#-features) • [Tech Stack](#-tech-stack) • [Architecture](#-architecture) • [Deployment](#-deployment) • [API Documentation](#-api-endpoints) • [Testing](#-testing-tdd-approach) • [AI Usage](#-my-ai-usage)
...
| **passlib + bcrypt** | 4.0.1 | Password hashing and security |
| **psycopg2** | 2.9.9 | PostgreSQL database adapter |
| **Pytest** | 8.1.1 | Testing framework for TDD |
...
| **Vite** | 5.2.8 | Build tool with Hot Module Replacement |
...
| **Nginx** | 1.25 | Web server for frontend |
...
│  │  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐ │ │
│  │  │ Auth Routes  │  │ Sweet Routes │  │  Inventory  │ │ │
│  │  │   /register  │  │   /sweets    │  │  /purchase  │ │ │
│  │  │   /login     │  │   /search    │  │  /restock   │ │ │
│  │  └──────────────┘  └──────────────┘  └─────────────┘ │ │
│  │                                                        │ │
│  │  ┌──────────────────────────────────────────────────┐ │ │
...
│  │                                                        │ │
│  │  ┌──────────────────────────────────────────────────┐ │ │
│  │  │    SQLAlchemy ORM (Models: User, Sweet)          │ │ │
│  │  └──────────────────────────────────────────────────┘ │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
                            ↕ SQL
...
1. **User Authentication Flow**
   ```
   User → Login Form → POST /api/login → Validate Credentials 
   → Generate JWT → Return Token → Store in localStorage
   ```

2. **Sweet Purchase Flow**
   ```
   User → Purchase Button → POST /api/sweets/{id}/purchase 
   → Verify JWT → Check Stock → Deduct Quantity → Update DB → Return Success
   ```

3. **Admin Operations Flow**
   ```
   Admin → Add/Edit Sweet Form → POST/PUT /api/sweets 
   → Verify JWT + Admin Role → Validate Data → Save to DB → Return Response
   ```
...
### 1. ☁️ Cloud Deployment (Production)
...
- Security groups configured (ports 80, 8001, 22)
...
#### i) 🐳 Docker & Docker Compose (Recommended)
...
| 📦 **Docker Compose** | [Installation Guide](https://docs.docker.com/compose/install/) (included in Desktop) |
...
2. **Start all services**
   ...
   *This command will:*
   - 🗄️ Build and start PostgreSQL database container
...
#### ii) 🔧 Local Environment (Manual Setup)
...
### Production Considerations (Cloud Deployment)
